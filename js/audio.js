...
function initAudioEvent() {
    ...
    audioPlayer.addEventListener('click', function () {
        ...
        if (audio.paused) {
            // 开始播放当前点击的音频
            audio.play();
            audioPlayer.src = './static/play.png';
        } else {
            audio.pause();
            audioPlayer.src = './static/stop.png';
        }
    }, false);
...
function dragProgressDotEvent(audio) {
    ...
    function down(event) {
        if (!audio.paused || audio.currentTime != 0) { // 只有音乐开始播放后才可以调节，已经播放过但暂停了的也可以
            flag = true;
            //相对于 包含块 的左侧偏移量
            position.oriOffsetLeft = dot.offsetLeft;
            //相对于 视口 的X坐标
            position.oriX = event.touches ? event.touches[0].clientX : event.clientX; // 要同时适配mousedown和touchstart事件
            position.maxLeft = position.oriOffsetLeft; // 向左最大可拖动距离
            position.maxRight = document.getElementById('progressBarBg').offsetWidth - position.oriOffsetLeft; // 向右最大可拖动距离
    ...
    function move(event) {
        if (flag) {
            ...
            var length = clientX - position.oriX; //鼠标拖动事件的X坐标值 - 鼠标按下事件的X坐标值
            ...
            var progressBarBg = document.getElementById('progressBarBg');
            var pgsWidth = parseFloat(window.getComputedStyle(progressBarBg, null).width.replace('px', ''));
            var rate = (position.oriOffsetLeft + length) / pgsWidth;
            audio.currentTime = audio.duration * rate;
            updateProgress(audio);
        }
    }
 ...
 */
//audio.currenttime 当前时间。audio.duration 总时间。以秒计。
...
function audioEnded() {
    ...
    document.getElementById('audioPlayer').src = './static/stop.png';
}
